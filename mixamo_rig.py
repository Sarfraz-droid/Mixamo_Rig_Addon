...
def _bake_anim(self):
    ...
    scn.frame_set(int(fs))
    bpy.context.view_layer.update()
        
    # bake NLA strips
    print("Baking, frame start:", fs, ",frame end", fe)
    bpy.ops.nla.bake(frame_start=int(fs), frame_end=int(fe), step=1, only_selected=True, visual_keying=False, clear_constraints=False, clear_parents=False, use_current_action=False, clean_curves=False, bake_types={'POSE'})
